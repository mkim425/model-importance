...
p1 <- dat %>% 
    ...
    labs(x="b (prediction of forecaster 3)", y="Expected  Importance", 
         title = bquote(bold("(a) Expected Importance based on MSPE"))) +
...
p2 <- simdat %>% rename(Model = model) %>% 
        ...
        geom_line(aes(linetype=Model)) +
        scale_linetype_manual(values=c("dashed","twodash", "solid")) +
        labs(x="b (bias of forecaster 3)", y="Expected  Importance", 
         title = bquote(bold("(b) Expected Importance based on MWIS"))) +
        theme(legend.position = "bottom") + 
        theme(axis.text=element_text(size=12),
              axis.title=element_text(size=14),
              strip.text = element_text(size = 12),
