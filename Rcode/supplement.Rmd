...
rank_lasomo_eq <- df_rank %>% 
        ...
        # Add labels for each line at the last point
        geom_text(aes(label = model), 
                  data = . %>% filter(Metric == "lasomo_eq_rank"),  # Place label at the last point
                  hjust = 0,  
                  size = 5)   +
        geom_text(aes(label = model), 
                  data = . %>% filter(Metric == "WIS_rank"),  # Place label at the last point
                  hjust = 1,  
                  size = 5)   +
        labs(title = bquote("(a) WIS vs. " * Phi[eq]^{lasomo}),
             x = "") +
...
rank_lasomo_wt <- df_rank %>% 
        ...
        geom_text(aes(label = model), 
                  ...
                  hjust = 1,  
                  size = 5)   +
        labs(title = bquote("(b) WIS vs. " * Phi[wt]^{lasomo}),
             x = "", y="") +
        theme(legend.position = "none",
              axis.title = element_text(size = 20),
              axis.text.x = element_text(size = 18),
...
rank_lomo_eq <- df_rank %>% 
        ...
        geom_text(aes(label = model), 
                  ...
                  hjust = 1,  
                  size = 5)   +
        labs(title = bquote("(c) WIS vs. " * Phi[eq]^{lomo})) +
        theme(legend.position = "none",
              axis.title = element_text(size = 20),
              axis.text.x = element_text(size = 18),
              axis.text.y = element_text(size = 16),
