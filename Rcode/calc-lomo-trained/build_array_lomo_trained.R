...
build_array_lomo_trained <- function(dat, truth){
        ...
        ## calculate importance score lomo trained for multiple locations at once
        # result_list <- importance_score_lomo_trained(dat, truth)
        
        ## calculate importance score lomo trained for single location
        result_list <- importance_score_lomo_trained_per_loc(dat, truth)
        ...
        saveRDS(arr_list, paste0(path, "/output/array-h",h,"-loc",incl_locations,"-",dates,".rds"))
} 
