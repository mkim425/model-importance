...
importance_score_shapley_trained <- function(forecast_data, truth){
        current_forecasts_date <- max(forecast_data$forecast_date)
        
        # Models that should be included in trained ensemble; 
        # if a model doesn't appear in the training dataset, 
        # we don't have any historical data on its performance, 
        # so there is no way to estimate its weight.
        # incl_models <- forecast_data %>% 
        ...
        df_all <- left_join(idx_wt, idx_wis, by=c("model", "location")) %>% 
