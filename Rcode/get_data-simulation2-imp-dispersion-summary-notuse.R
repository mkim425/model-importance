...
simu_data_f3dispersion <- function(s){
        ...
        ave.imp <- data %>%
                 dplyr::select(model, importance, dispersion) %>%
                 dplyr::group_by(model) %>%
                 dplyr::summarise(f3_sharpness=s,
...
write.csv(list.df.23qt, 
          paste0(base_path,"simulation_f3sharp-1000rep_july21.csv"), 
          row.names = FALSE)
saveRDS(list.df.23qt, 
        paste0(base_path,"simulation_f3sharp-1000rep_july21.rds"))
