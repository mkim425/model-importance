...
p1 <- dat %>% 
    ...
    labs(x="b (prediction of forecaster 3)", y="Importance", title = "(a)") +
...
p2 <- simdat %>% rename(Model = model) %>% 
        ...
        geom_line(aes(linetype=Model)) +
        scale_linetype_manual(values=c("dashed","twodash", "solid")) +
        labs(x="b (bias of forecaster 3)", y="Importance", title = "(b)") +
        theme(legend.position = "bottom")
```
...
pdf(file="../Plot/plot-simulation-settingA.pdf",
    width = 10,
    height = 5)
p
dev.off()
...
```{r}
pdf(file="../Plot/plot-simulation-settingB.pdf",
    width = 10,
    height = 5)
p3
dev.off()
