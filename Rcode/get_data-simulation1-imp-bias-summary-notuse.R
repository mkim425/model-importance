...
# Importance scores of 3 forecasters with fixed dispersion but different biases of Forecaster 3
...
simu_data_f3bias <- function(b){
        ...
        data_3a.23qt <- df.23qt.full %>%
                ...
                dplyr::select(model, importance, wis, dispersion, overpred, underpred, error, MAE, t)
        
        ave.imp <- data_3a.23qt %>%
                dplyr::select(model, importance, error) %>%
                dplyr::group_by(model) %>%
                dplyr::summarise( f3_bias=b,
                           bias=mean(error),
