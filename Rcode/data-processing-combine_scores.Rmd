...
# forecast_data <- readRDS(paste0(basepath,"/main-data//forecast_data_aligned_death_10models_nov20-nov22.rds")) %>%  select(-c(2,6,7)) %>% 
...
# 
# truth_data <- readRDS(paste0(basepath,"/main-data/truth_data_upto2022.rds")) # %>% select("location", "target_end_date", "value")
# 
...
# forecast_data <- readRDS(paste0(basepath,"/main-data/forecast_data_aligned_death_10models_nov20-nov22.rds")) %>% select(-c(2)) %>% 
#         rename(forecast_date = reference_date) %>% # Change the column names  
#         arrange(forecast_date, model)
# truth_data <- readRDS(paste0(basepath,"/main-data/truth_data_upto2022.rds")) 
# 
...
# saveRDS(scores, paste0(basepath, "/main-data/scores_wis.rds"))
```
...
# scores_wis <- readRDS(paste0(basepath, "/main-data/scores_wis.rds"))
# wis <- scores_wis %>% select(model, location, horizon, forecast_date, wis)
shapley_trained <- readRDS(paste0(basepath,"/main-data/shapley_trained_completed.rds")) %>%
        rename(ImpScore_shapley_wt = ImportanceScore) %>%
        transform(horizon=as.character(horizon),
                  forecast_date = as.Date(forecast_date))

shapley_untrained <- readRDS(paste0(basepath,"/main-data/shapley_untrained_completed.rds")) %>%
        rename(ImpScore_shapley_eq = ImportanceScore) %>%
        transform(horizon=as.character(horizon),
                  forecast_date = as.Date(forecast_date))

lomo_untrained <- readRDS(paste0(basepath,"/main-data/lomo_untrained_completed.rds")) %>%
        rename(ImpScore_lomo_eq = ImportanceScore) %>%
        transform(horizon=as.character(horizon),
                  forecast_date = as.Date(forecast_date))

lomo_trained <- readRDS(paste0(basepath,"/main-data/lomo_trained_completed.rds")) %>%
...
# 
# saveRDS(df_all_scores_extended, paste0(basepath,"/main-data/df_all_scores_extended.rds"))
```
...
saveRDS(df_all_scores_extended_NAworst, paste0(basepath,"/main-data/df_all_scores_extended_NAworst.rds"))
```
...
saveRDS(df_all_scores_extended_NAavg, paste0(basepath,"/main-data/df_all_scores_extended_NAavg.rds"))
...
Drop all NAs, and add relWIS

```{r}
df_all_scores_extended <- readRDS(paste0(basepath,"/main-data/df_all_scores_extended.rds"))

df_all_scores_extended_NAdrop <- df_all_scores_extended %>% drop_na()
saveRDS(df_all_scores_extended_NAdrop, paste0(basepath,"/main-data/df_all_scores_extended_NAdrop.rds"))
```
